...
describe('<Jokes />', () => {
  const joke = {
    question: 'Why did the skeleton write tests?',
    answer: 'To see if she did anything bone-headed.',
  };
  ...
  it('shows a joke', () => {
    ...
    expect(root.find('h1').text()).equal(joke.question);
  });
  ...
  it('shows the answer when state.answered=true', () => {
    ...
    expect(root.find('h2').text()).to.equal(joke.answer);
  });

  it('when tapped, sets state.answered=true', () => {
    root.setState({ joke, answered: false });
    root.simulate('click');
    expect(root.state().answered).to.be.true; // eslint-disable-line no-unused-expressions
  });
});
